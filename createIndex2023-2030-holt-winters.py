...
def appendToCSV(val):
    with open('EHI-2023-2030-ARIMA-holt-winters.csv', 'a', newline='') as file:
...
df = pd.read_csv('./finalArimaDatasets/holt_winters_consolidated_forecast_2023_2030.csv')
df = df[sorted(df.columns)]
df = df.drop(columns=['DATE', 'GDP'])

with open('EHI-2023-2030-ARIMA-holt-winters.csv', 'w') as f:
    writer = csv.writer(f)
